...
def load_potential_3(train_size, test_size, dimension=2):

    logging.debug(f"loading potential_3 trian_size: {train_size} test_size: {test_size} dimension: {dimension}")

    X = np.load("data/NF3/nf3.npy").astype(np.float32)
    X_densities = np.loadtxt("data/NF3/NF3_densities.csv").astype(np.float32)
...
def generate_potential_3_dataset(train_size, test_size, dimension=2):

    logging.debug(f"generating potential_3 trian_size: {train_size} test_size: {test_size} dimension: {dimension}")
    ...
    np.savetxt(fname="data/NF3/NF3.csv", delimiter=" ", X = samples)
