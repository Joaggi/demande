...
def generate_model_inverse_maf(setting):
    ...
    permutation = tf.cast(np.concatenate((np.arange(input_shape / 2, input_shape), np.arange(0, input_shape / 2))),
                          tf.int32)[:input_shape]

    base_dist = tfd.MultivariateNormalDiag(loc=tf.zeros(input_shape),
                                           scale_diag=tf.ones(input_shape))  # specify base distribution

    bijectors = []

    for _ in range(0, n_layers):
        bijectors.append(tfb.Invert(tfb.MaskedAutoregressiveFlow(shift_and_log_scale_fn= \
                                                                     Made(params=2, hidden_units=hidden_shape,
                                                                          activation="relu"))))
